...
namespace FPA
{
	...
	public class DD_CoordinateAxis : DD_DrawGraphic
	{
		...
		private DD_DataDiagram m_DataDiagram = default;
		private RectTransform m_CoordinateRectT = default;
		private GameObject m_LinesPreb = default;
		private GameObject m_MarkTextPreb = default;
		...
		{
			get => INCH_PER_CENTIMETER * m_DataDiagram.m_CentimeterPerMark * Screen.dpi;
		}
		...
		private void ChangeScale(float ZoomX, float ZoomY)
		{
			...
			Vector2 viewSize = new Vector2(m_CoordinateAxisViewRange.width * rangeSize.x, m_CoordinateAxisViewRange.height * rangeSize.y);
		...
		private void OnMove(object sender, DD_MoveEventArgs e)
		{
			...
			CoordinateeZeroPointChangeEvent(this, new DD_CoordinateZeroPointChangeEventArgs(CoordinateToPixel(dis)));
			UpdateGeometry();
		}
		...
		{
			SetMarkText(markText, new Rect(new Vector2(0, m_CoordinateRectT.rect.y), new Vector2(m_CoordinateRectT.rect.x, m_MinMarkTextHeight)), null, false);
		}
		...
		private void DrawHorizontalMark(VertexHelper vh, Rect coordinateRect)
		{
			...
			float[] marksVals = CalcMarkVals(markLevel, m_CoordinateAxisViewRange.y, m_CoordinateAxisViewRange.y + m_CoordinateAxisViewSizeY);
			float[] marksPixel = MarkValsToPixel(marksVals, m_CoordinateAxisViewRange.y, m_CoordinateAxisViewRange.y + m_CoordinateAxisViewSizeY, coordinateRect.y, coordinateRect.y + coordinateRect.height);
