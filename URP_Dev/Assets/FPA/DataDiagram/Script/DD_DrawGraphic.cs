...
namespace FPA
{
	public class DD_DrawGraphic : MaskableGraphic
	{
		...
		{
			if (IsPointHorizontalInRect(p, rect) != 0)
			{
				return false;
			}
			if (IsPointVerticalityInRect(p, rect) != 0)
			{
				return false;
			}
		...
		protected void DrawPoint(VertexHelper vh, Vector2 point, Color color, float thickness, float scaleX = 1, float scaleY = 1)
		{
		...
		protected void DrawHorizontalSegmet(VertexHelper vh, Vector2 startPoint, Vector2 endPoint, Color color, float thickness, float scaleX = 1, float scaleY = 1)
		{
		...
		protected void DrawVerticalitySegmet(VertexHelper vh, Vector2 startPoint, Vector2 endPoint, Color color, float thickness, float scaleX = 1, float scaleY = 1)
		{
		...
		protected void DrawHorizontalLine(VertexHelper vh, List<Vector2> points, Color color, float thickness, Rect range)
		{
			...
			{
				if (!IsPointInRect(points[i], range))
				{
					points.RemoveAt(i);
					continue;
				}

				if (!IsPointInRect(points[i + 1], range))
				{
					points.RemoveAt(i + 1);
					i++;
					continue;
		...
		protected void DrawTriangle(VertexHelper vh, Vector2 points, Color color, float thickness, float rotate, float scaleX = 1, float scaleY = 1)
		{
			float edge = (thickness / 3) * 2;
			Vector2 point1st = new Vector2((points.x + Mathf.Sin(Mathf.Deg2Rad * rotate) * edge) * scaleX, (points.y + Mathf.Cos(Mathf.Deg2Rad * rotate) * edge) * scaleY);
			Vector2 point2nd = new Vector2((points.x + Mathf.Sin(Mathf.Deg2Rad * (rotate + 120)) * edge) * scaleX, (points.y + Mathf.Cos(Mathf.Deg2Rad * (rotate + 120)) * edge) * scaleY);
			Vector2 point3rd = new Vector2((points.x + Mathf.Sin(Mathf.Deg2Rad * (rotate + 240)) * edge) * scaleX, (points.y + Mathf.Cos(Mathf.Deg2Rad * (rotate + 240)) * edge) * scaleY);
